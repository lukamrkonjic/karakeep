...
| `apps/web/components/dashboard/UploadDropzone.tsx` | Fixed a real client bug: it hardcoded `assetType: contentType === "application/pdf" ? "pdf" : "image"` — any non-PDF file, including videos, got mislabeled `"image"`. Now checks for `video/*` too. Updated the drop placeholder text. |
...
| `apps/workers/workers/assetPreprocessingWorker.ts` | Added `extractAndSaveVideoThumbnail()` (ffmpeg `-frames:v 1 -update 1` frame grab, scaled to preserve aspect ratio, capped at 1280px wide), mirroring the existing PDF-screenshot pattern. `run()` branches early on `req.data.assetId` to target this specific-asset job type before falling through to the existing primary-asset logic (unchanged). Its own asset-type gate uses the same `linkVideo`-OR-(`bookmarkAsset`-and-video-`contentType`) check as the admin query above (kept in sync manually — no shared helper, since the two need slightly different data shapes) — this went through two iterations: `assetType === "linkVideo"` only (missed directly-uploaded video bookmarks) → `contentType`-only (missed `linkVideo` assets with a missing/wrong `contentType`, e.g. bulk-imported ones) → the current OR of both. | Additive — a new function plus one early branch at the top of `run()`. Should merge cleanly unless upstream restructures `run()`'s dispatch, in which case keep the `req.data.assetId` branch and the new function. |
