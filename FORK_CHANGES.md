...
| `apps/web/components/dashboard/bookmarks/MasonryMediaCard.tsx` | Eagle/Pinterest-style borderless, media-only masonry tile with hover-dim + white action icons; the hover title stays on one line (the full title is its tooltip), and image tiles get the `ImagePeek` magnifier (not in bulk edit) |
...
| `apps/web/components/dashboard/preview/BookmarkPreview.tsx` | New `variant` prop: `"modal"` wraps a picture/video with `MediaFitPreview` and gives everything else an explicit 90vw×90vh box; `"page"` (default) fills its parent as before. `<BookmarkListChips>` is a section above Tags (the title-row badges are gone), and a video note's text shows in the details panel. The title is one line (full title as tooltip); where the bookmark came from is a "Source" section under it, a clickable short URL (was a small "View original" link); `onClose` goes on to `MediaFitPreview`. The original wide layout's JSX is re-indented inside the `media ?` ternary, so a conflict there is usually indentation only |
